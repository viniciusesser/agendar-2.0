...
import { AgendaBanners } from "@/components/ui/AgendaBanners"; // ← NOVO
...
function organizarAgendamentosSemSobreposicao(itensGrid: any[]) {
  ...
  clusters.forEach((cluster) => {
    const colunasUnicas = Array.from(
      new Set(cluster.map((ag) => ag._colIndex))
    ).sort();
    cluster.forEach((ag) => {
...
export default function AgendaPage() {
  ...
  const listaAgendamentos = Array.isArray(dadosApi)
    ? dadosApi
    : dadosApi?.agendamentos || [];
  const listaBloqueios = Array.isArray(dadosApi)
    ? []
    : dadosApi?.bloqueios || [];
  ...
  const mutationExcluirBloqueio = useMutation({
    mutationFn: async (id: string) => api.delete(`/bloqueios/${id}`),
    onSuccess: () => {
  ...
  const handleExcluirBloqueio = (bloqueio: any) => {
    if (
      window.confirm(
        `Deseja remover o bloqueio de ${bloqueio.profissional?.nome} e liberar este horário?`
      )
    ) {
  ...
  const startHour = cfg?.horario_abertura
    ? parseInt(cfg.horario_abertura.split(":")[0])
    : 7;
  const endHour = cfg?.horario_fechamento
    ? parseInt(cfg.horario_fechamento.split(":")[0])
    : 21;
  ...
  const bloqueiosFormatados = listaBloqueios.map((b: any) => ({
    ...
    isBloqueio: true,
    duracao_min:
      (new Date(b.data_hora_fim).getTime() -
        new Date(b.data_hora_inicio).getTime()) /
      60000,
  }));

  const itensOrganizados = organizarAgendamentosSemSobreposicao([
    ...listaAgendamentos,
    ...bloqueiosFormatados,
  ]);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-bg-default antialiased pb-24">
      ...
      <header className="bg-surface border-b border-border-default px-4 pt-10 pb-4 sticky top-0 z-[1000] shadow-sm">
        <div className="flex items-center justify-between max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => mudarDia(-1)}
              className="p-2 text-primary-action active:scale-90 transition-transform"
            >
      ...
      {/* ↓ BANNERS: Feriado (roxo) + Ausências (vermelho) — substituímos os dois blocos antigos por este componente */}
      <AgendaBanners data={dataSelecionada} bloqueios={listaBloqueios} />
            ...
            <div className="relative">
              {timeSlots.map((time) => {
                ...
                return (
                  <div
                    key={time}
                    className="flex border-b border-bg-default"
                    style={{ height: `${slotHeightPixels}px` }}
                  >
              ...
              <div className="absolute top-0 left-[64px] right-0 bottom-0 pointer-events-none">
                <div className="relative w-full h-full">
                  ...
                  {itensOrganizados.map((ag: any) => {
                    const duracao =
                      ag.duracao_min || ag.servico?.duracao_min || 60;
                      ...
                      return (
                        ...
                        >
                          <div className="bg-surface/90 px-3 py-1.5 rounded-lg shadow-sm text-center flex flex-col items-center max-w-full">
                            <UserX
                              size={14}
                              className="text-text-muted mb-0.5 group-hover:text-status-error transition-colors"
                            />
                            <p className="text-[10px] font-black text-text-secondary uppercase truncate w-full">
                              {ag.profissional?.nome}
                            </p>
                            {ag.motivo && (
                              <p className="text-[9px] font-bold text-status-error uppercase truncate w-full mt-0.5">
                                {ag.motivo}
                              </p>
                            )}
                          </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div
                            className={`shrink-0 w-2 h-2 rounded-full border border-white/50 ${
                              statusColors[ag.status] || "bg-white/20"
                            }`}
                          />
                        </div>

                        {duracao >= 30 && (
                          <p className="text-[11px] font-bold text-white/80 truncate drop-shadow-sm">
                            {ag.servico?.nome}
                          </p>
                        )}
                      </div>
